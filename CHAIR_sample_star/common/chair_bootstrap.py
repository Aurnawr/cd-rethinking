...
bootstrap CIs (B=10,000) for the Sample* leg (sample_star, VCD, SID,
...
def main():
    ...
    ap.add_argument("--out", default=str(HERE.parent / "CHAIR_ANALYSIS_SAMPLESTAR" / "chair_bootstrap.json"))
    args = ap.parse_args()

    results = {}
    for model in ("llava", "qwen"):
        image_ids = set()
        for method in ("sample_star", "vcd", "sid", "icd"):
