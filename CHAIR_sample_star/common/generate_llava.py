"""
generate_llava.py -- generation for LLaVA-1.5-7B, Sample* edition. Two
modes, each with a choice of final-token-selection rule:

  --mode sample_star
        Single branch (expert only), NO contrastive decoding, but the
        Adaptive Plausibility Constraint (APC) mask IS applied to the raw
        expert distribution before sampling: cutoff = log(beta) + max(E),
        candidates with E < cutoff get -inf. This is "Sample*"/"Sample-dagger"
        from Yin et al.'s "Mirage of Performance Gains" -- APC alone, no
        amateur branch, no contrastive term -- meant to show how much of
        CD's apparent effect the plausibility mask accounts for on its own.
        Captures the expert's own top-10 tokens/logits at every step, plus
        the APC cutoff and each candidate's survival flag (same shape as
        the two-branch capture, minus the amateur/contrastive fields).
  ...
  --decode {greedy,sample}   (applies to BOTH modes)
        greedy : chosen_id = argmax(scored)
        sample : chosen_id = multinomial(softmax(scored)), temperature 1.0
                 -- the real run plan uses sample everywhere (this whole
                 package's point is direct-sampling comparisons); greedy is
                 kept for quick sanity spot-checks only.

cd_beta = 0.1 here (NOT the 0.2 used in the sibling CHAIR_sampling+ICD
package) -- matching the Mirage paper's own Table 6, which evaluates
Sample, Sample-dagger, VCD(sample), and SID(sample) all under beta=0.1
(VCD's own paper default) so the comparison isolates what the amateur
branch adds ON TOP of the plausibility mask, at a fixed mask strength.
Everything else matches the sibling package: cd_alpha=1.0, VCD
noise_step=500, max_new_tokens=256, prompt="Describe this image in
detail.", same 500 image_ids_500.json.
...
REPO = HERE.parent                              # CHAIR_sample_star/
...
MAX_NEW_TOKENS = 256
CD_ALPHA = 1.0
CD_BETA = 0.1  # Mirage paper's Table 6 convention -- see module docstring
...
def main():
    ...
    print(f"[llava] mode={args.mode} method={args.method} decode={args.decode} "
          f"seed={args.seed} n={args.n_images} cd_beta={CD_BETA}", flush=True)
    ...
    for image_id in IMAGE_IDS[: args.n_images]:
        ...
        for step in range(MAX_NEW_TOKENS):
            ...
            if two_branch:
                ...
                # Sample*: APC mask applied directly to the expert distribution,
    ...
    out_f.close()
    print(f"[llava] DONE {args.mode}/{args.method or 'sample_star'}/{args.decode} -> {args.out}", flush=True)
