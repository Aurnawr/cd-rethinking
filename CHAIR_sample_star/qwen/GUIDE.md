...
Same reasoning as the top-level `GUIDE.md`: cd_beta=0.1 uniformly across
sample_star/VCD/SID/ICD, matching the Mirage paper's Table 6 (NOT the
sibling `CHAIR_sampling+ICD` package's 0.2) -- see that file's Section 2
for the full rationale.

| Parameter | Value |
|---|---|
| `cd_alpha` | 1.0 |
| `cd_beta` (APC) | **0.1** (Mirage paper Table 6 convention -- see top-level `GUIDE.md`) |
| `max_new_tokens` | 256 |
...
| ICD `cd_alpha`/`cd_beta` | 1.0 / 0.1 (matches this package's Sample*/VCD/SID convention) |
