...
Then, from this qwen/ directory:
  ...
  python spawn_qwen_jobs.py --full      # 500 images, seed 0, Sample*+VCD+SID+5 ICD prompts
                                         # (8 jobs total)
...
def main():
    ...
    gen = modal.Function.from_name("cd-rethink-sample-star", "generate_qwen")
    gen_icd = modal.Function.from_name("cd-rethink-sample-star", "generate_qwen_icd")

    calls = {}

    for seed in seeds:
        out = f"captures/qwen_sample_star_seed{seed}.jsonl"
        call = gen.spawn(mode="sample_star", method="", decode="sample", seed=seed, out_name=out, n_images=n_images)
