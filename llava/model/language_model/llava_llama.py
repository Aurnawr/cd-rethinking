...
try:
    # transformers >=4.46 ships its own native "llava" model type, which
    # collides with this repo's custom registration (raises ValueError:
    # 'llava' is already used by a Transformers config, pick another name).
    # Just importing the `llava` package (e.g. `llava.utils` for an
    # unrelated helper) triggers this, so make it a no-op when that happens
    # instead of crashing every script that touches the package.
