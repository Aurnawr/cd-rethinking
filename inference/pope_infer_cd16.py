...
def eval_model(args):
    ...
    # Enable AnyRes AFTER the cd/sid sampling patches (patches target different
    # methods: generation sample/greedy vs model forward/prepare, so order is safe).
    anyres.apply_anyres_patches()

    disable_torch_init()
    model_path = os.path.expanduser(args.model_path)
    model_name = get_model_name_from_path(model_path)
    tokenizer, model, image_processor, context_len = load_pretrained_model(model_path, args.model_base, model_name)
    anyres.load_image_newline(model, model_path)
    ...
    for line in tqdm(questions, desc=os.path.basename(answers_file)):
        ...
        image_tensor = anyres.process_images_anyres([image], image_processor, model.config)[0]
