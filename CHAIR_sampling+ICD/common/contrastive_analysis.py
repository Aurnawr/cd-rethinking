...
baseline, vcd, sid, icd_p1, icd_p2, icd_n1, icd_n2, icd_p3) and reports, PER
...
draws on the same 500 images):
  ...
  (3) CHAIR-S / CHAIR-I for every method x seed present, including baseline
      -- this is the direct-sampling replacement for the old greedy
      baseline row, so it can be compared against VCD/SID/ICD under the
      SAME (sampling) decoding rule.

Usage:
  python contrastive_analysis.py --methods baseline vcd sid icd_p1 icd_p2 icd_n1 icd_n2 icd_p3 --seeds 0 1
"""
