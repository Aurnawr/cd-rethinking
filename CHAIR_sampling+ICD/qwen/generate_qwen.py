...
avoids it instead of working around it).
  ...
  --mode baseline                      single branch (expert only)
...
cd_beta=0.2, max_new_tokens=256, prompt="Describe this image in detail.",
same 500 images.
...
def main():
    ...
    for image_id in IMAGE_IDS[: args.n_images]:
        ...
        for step in range(MAX_NEW_TOKENS):
            ...
            if two_branch:
                ...
                steps_out.append({"step": step, "chosen_id": chosen_id,
                                   "expert_top_ids": ei.tolist(), "expert_top_logits": [round(v, 4) for v in ev.tolist()]})
    ...
    out_f.close()
    print(f"[qwen] DONE {args.mode}/{args.method or 'baseline'}/{args.decode} -> {args.out}", flush=True)
